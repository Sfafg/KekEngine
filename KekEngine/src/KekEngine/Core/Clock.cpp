...
namespace Kek
{
	...
	template <TimeFormat TFormat>
	Clock<TFormat>::Clock()
	{
		Restart();
	}
	template <TimeFormat TFormat>
	void Clock<TFormat>::Restart()
	{
		start = std::chrono::high_resolution_clock::now();
	}
	template <TimeFormat TFormat>
	float Clock<TFormat>::operator()() { return GetTime(); }
	template <TimeFormat TFormat>
	Clock<TFormat>::operator float() { return GetTime(); }
	template <TimeFormat TFormat>
	float Clock<TFormat>::GetTime()
	{
