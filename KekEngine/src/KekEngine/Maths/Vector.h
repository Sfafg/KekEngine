...
namespace Kek
{
	...
	struct vec : public vec_union<TSize, T>
	{
		...
		vec<TSize, T> Round() { for(T& v : this->value) v = Maths::Round(v); return *this; }
		vec<TSize, T> Ceil() { for(T& v : this->value) v = Maths::Ceil(v); return *this; }
		vec<TSize, T> Abs() { for(T& v : this->value) v = Maths::Abs(v);  return *this; }
		vec<TSize, T> Sign() { for(T& v : this->value) v = Maths::Sign(v); return *this; }
	...
	namespace Vector
	{
		...
		template <unsigned int TSize, typename T> vec<TSize, T> Lerp(const vec<TSize, T>& a, const vec<TSize, T>& b, float k) { return Maths::Lerp(a, b, k); }
		...
		{
			vec<TSize, T> r;
			for(int i = 0; i < TSize; i++) r[i] = Maths::Random(min, max);
			return r;
		}
		template <unsigned int TSize, typename T>
		...
		{
			vec<TSize, T> r;
			for(int i = 0; i < TSize; i++) r[i] = Maths::Random(min[i], max[i]);
			return r;
		}
		template <unsigned int TSize, typename T>
		vec<TSize, T> Unit()
